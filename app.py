...
def capture_and_process_video():
    ...
    if not cap.isOpened():
        emit('error', {'message': 'Error: Could not open video capture.'})
        return
...
# Route to render the direction.html template
@flask_app.route("/direction")
def direction():
...
def start_captioning():
    ...
    return jsonify({"status": "started", "mode": "video"})

# Route to stop captioning process
@flask_app.route("/stop_captioning", methods=["POST"])
def stop_captioning():
    global capturing, video_thread
    capturing = False
    video_thread = None  # Reset the video thread
    return jsonify({"status": "stopped", "mode": "video"})
...
def start_audio_transcription():
    ...
    return jsonify({"status": "started", "mode": "audio"})
...
def stop_audio_transcription():
    global running, audio_thread
    running = False
    audio_thread = None  # Reset the audio thread
    return jsonify({"status": "stopped", "mode": "audio"})
